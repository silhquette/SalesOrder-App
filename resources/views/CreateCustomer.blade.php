                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form method="POST" action="{{ isset($create) ? route('customer.store') : route('customer.update', $customer->code) }}" id="input-form">
                        <div class="flex justify-center mt-4">
                            <div class=" grid grid-cols-2 gap-6 w-2/3">
                                @csrf
                                @if (!isset($create))
                                    @method('PATCH')
                                @endif
                                ...
                                <div class="mx-4">
                                    ...
                                    <x-text-input id="npwp" class="block mt-1 w-full" type="text" name="npwp" value="{{ old('npwp') ? old('npwp') : (isset($customer->npwp) ? $customer->npwp : '') }}" required autofocus/>
                                ...
                                <div class="mx-4">
                                    ...
                                    <x-text-input id="email" class="block mt-1 w-full" type="text" name="email" value="{{ old('email') ? old('email') : (isset($customer->email) ? $customer->email : '') }}" required autofocus />
                                    <x-input-error :messages="$errors->get('email')" class="mt-2" />
                                </div>
                            
                                <!-- Address to send -->
                                <div class="mx-4">
                                    <x-input-label for="address" :value="__('Address to send')" class="after:content-['*'] after:ml-0.5 after:text-red-500" />
